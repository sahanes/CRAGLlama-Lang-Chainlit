...
class RealtimeClient(RealtimeEventHandler):
    ...
    async def _call_tool(self, tool):
        try:
            ...
            #     query = json_arguments.get("query")
            #     result = await tool_config["handler"](query)
            if tool["name"] == "rag_query":
                query = json_arguments.get("query")
                if query is None:
                    raise ValueError("Missing 'query' argument for 'rag_query'")
                result = await tool_config["handler"](query)
                output=result
                # Send the result directly if it's a string
            ...
            logger.error(f"Tool call error: {json.dumps(error_message)}")
            await self.realtime.send("conversation.item.create", {
                "item": {
                    "type": "function_call_output",
                    "call_id": tool["call_id"],
                    "output": json.dumps(error_message),
                }
            })
